...
public class WeappApiAction {
    ...
    public String getExaminfo(@RequestBody Map<String, String> conditionMap) {
        ...
        if (!weappApiService.checkStuValid(name, idnumber)) {
            return JSON.toJSONString(new BaseResponse<>(1, "找不到对应的学生信息"));
        }
        NdStudent student = studentService.getStudentByIdNumber(idnumber);

        Map<String, Object> examinfo = weappApiService.getExaminfoByNumber(student.getNumber(), year, Integer.valueOf(term));
